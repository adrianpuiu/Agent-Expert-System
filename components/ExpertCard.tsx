...
import { Database, Server, Wifi, Layout, Brain, Activity, Eye, Zap, MessageSquare, Upload } from 'lucide-react';
...
const ExpertCard: React.FC<ExpertCardProps> = ({ expert, onChat, onImprove, onView, onTrain }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col justify-between transition-all hover:shadow-md">
      <div>
        <div className="flex justify-between items-start mb-4">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
            expert.status === ExpertStatus.ACTIVE ? 'bg-orange-100 text-orange-700' :
            expert.status === ExpertStatus.LEARNING ? 'bg-purple-100 text-purple-700 animate-pulse' :
            expert.status === ExpertStatus.THINKING ? 'bg-blue-100 text-blue-700 animate-pulse' :
